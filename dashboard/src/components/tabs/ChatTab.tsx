...
import { MOCK_MESSAGES, getConversations, getClientDocs, getProjectDocs, type Project, type Message } from "@/lib/mock";
          ...
          <div className="flex flex-wrap gap-1.5">
            {allDocs.map((doc) => (
              ...
              >
                ...
                <span className="text-[11px] font-medium text-zinc-700 dark:text-zinc-300 truncate max-w-[100px]">{doc.name}</span>
              </div>
            ))}
          </div>
          <button className="w-full mt-3 px-3 py-2 rounded-lg border border-dashed border-zinc-200 dark:border-zinc-800 text-[11px] text-zinc-500 dark:text-zinc-600 hover:border-zinc-300 dark:hover:border-zinc-700 hover:text-zinc-700 dark:hover:text-zinc-400 transition-colors">
            + Ajouter un doc
          </button>
        </div>
